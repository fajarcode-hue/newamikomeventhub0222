...
@section('content')
    ...
    <section class="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-12">
        ...
        <div class="flex-1 relative">
            ...
            <img src="{{ asset('concert.png') }}" alt="Concert" class="rounded-[2rem] shadow-2xl relative z-10 w-full object-cover aspect-[4/5] object-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            @empty
                <div class="col-span-full text-center py-12">
                    <p class="text-slate-400 font-medium text-lg">Belum ada event tersedia saat ini.</p>
                </div>
            @endforelse
        </div>
    </section>

    {{-- TAMBAHAN: Section Partners / Sponsor --}}
    <section class="bg-slate-50 border-t border-b border-slate-100 py-20">
        <div class="max-w-7xl mx-auto px-6 text-center">
            <p class="text-xs uppercase tracking-widest font-black text-slate-400 mb-3">Official Partners</p>
            <h2 class="text-3xl font-extrabold text-slate-800 mb-12">Didukung oleh Mitra Terpercaya</h2>
            
            <div class="flex flex-wrap items-center justify-center gap-12 md:gap-20 opacity-60 hover:opacity-100 transition-opacity duration-300">
                @forelse($partners as $partner)
                    <div class="h-12 flex items-center justify-center grayscale hover:grayscale-0 transition-all duration-300" title="{{ $partner->name }}">
                        <img src="{{ asset('storage/' . $partner->logo_url) }}" alt="{{ $partner->name }}" class="max-h-full max-w-[140px] object-contain">
                    </div>
                @empty
                    <p class="text-slate-400 text-sm font-medium">Buka kerja sama kemitraan bersama Platform Event.</p>
                @endforelse
            </div>
        </div>
    </section>
